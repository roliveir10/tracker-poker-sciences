...
export async function parseImport({
	...
	await prisma.$transaction(async (tx) => {
		for (const t of parsed.tournaments) {
			...
			// Clear existing hands to avoid duplicates on re-parse
			await tx.hand.deleteMany({ where: { tournamentId: tournament.id } });
            ...
            for (const h of t.hands) {
                const created = await tx.hand.create({
                    data: {
                        ...
                        mainPotCents: (h as Partial<{ mainPotCents: number | null }>).mainPotCents ?? null,
                    },
                });

                if (h.actions && h.actions.length > 0) {
                    const toActionType = (t: 'fold' | 'check' | 'call' | 'bet' | 'raise' | 'push') => t;
                    const toStreet = (s: 'preflop' | 'flop' | 'turn' | 'river') => s;
                    for (const a of h.actions) {
                        await tx.action.create({
                            data: {
                                handId: created.id,
                                street: toStreet(a.street),
                                seat: a.seat ?? 0,
